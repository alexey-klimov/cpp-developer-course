/* Задание
 * Класс Rational уже умеет сообщать об ошибках деления на ноль, но всё ещё разрешает создавать
 * дроби с нулевым знаменателем. Доработайте конструктор класса Rational так, чтобы при попытке
 * сконструировать дробь с нулевым знаменателем, выбрасывалось исключение типа domain_error.
 */
...
public:
    ...
    Rational(int numerator, int denominator) : numerator_(numerator), denominator_(denominator) {
        if (denominator == 0) {
            throw domain_error("знаменатель равен нулю");
        }
        Normalize();
    }
